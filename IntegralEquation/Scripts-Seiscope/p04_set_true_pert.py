...
if __name__ == "__main__":
    ...
    lower_bound = (1.0 / (np.max(vp_vz) ** 2.0)) - (1.0 / (vp_compact ** 2.0))
    upper_bound = (1.0 / (np.min(vp_vz) ** 2.0)) - (1.0 / (vp_compact ** 2.0))

    xmax = 1.0
    zmax = (obj.b - obj.a)
    extent = [0, scale_fac_inv * xmax, scale_fac_inv * zmax, 0]

    plot(psi, extent, "True perturbation", 1, os.path.join(basedir, "Fig/psi.pdf"))
    plot(lower_bound, extent, "Lower bound", 1)
    plot(upper_bound, extent, "Upper bound", 1)
